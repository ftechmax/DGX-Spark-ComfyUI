...
# Under --bf16-vae the tensors reaching the 3D nodes are bf16, and every
# conversion upstream writes as `.cpu().numpy().astype(np.float32)` dies with
#
#   TypeError: Got unsupported ScalarType BFloat16
#
# The cast to fp32 lands on the NumPy side, one step too late: torch cannot hand
# a bf16 tensor to NumPy at all, so .numpy() raises before .astype() is ever
# reached. Casting with .float() first crosses the boundary as fp32 and the
# astype becomes redundant.
#
# Two files carry the idiom. They are one bug, so they are patched in one step:
#
#   nodes_mesh_postprocess.py  the voxel colour field, which BakeTextureFromVoxel
#                              trips over.
#   nodes_save_3d.py           save_glb()'s five float attributes (vertices, uvs,
#                              vertex colors, normals, tangents), plus the _img
#                              helper in mesh_item_to_glb_bytes() doing the same
#                              to the texture/ORM/normal/emissive maps, which come
#                              straight out of the bf16 VAE. Only vertex colors —
#                              PaintMesh output into MeshToFile3D — has actually
#                              been seen to crash, but all six take whatever dtype
#                              the graph hands them, so which one blows up is a
#                              property of the workflow, not of the code.
#
# `faces` is deliberately untouched: it converts .astype(np.int64), which the
# expressions below do not match and which bf16 never reaches anyway.
#
# Not something a COMFYUI_REF bump fixes: both are still in upstream master. The
# other way out is --fp32-vae, which gives up the bf16 VAE for the whole stack to
# route around a handful of conversions — see .env.example for why not.
#
# The greps around the seds are the point. A sed that quietly matches nothing
# would ship an unpatched image and the crash would come back mid-workflow. The
# counted ones for nodes_save_3d.py go further and assert that none of its five
# float32 sites survive, so a ref that renames or fixes even one of them fails
# the build here rather than shipping a half-patched exporter.
RUN set -eux; \
    ...
    f=/opt/ComfyUI/comfy_extras/nodes_save_3d.py; \
    test "$(grep -c '\.cpu()\.numpy()\.astype(np\.float32)' "$f")" = 5; \
    grep -q 'clamp(0\.0, 1\.0)\.cpu()\.numpy()' "$f"; \
    sed -i -E \
        -e 's/([A-Za-z_]+)\.cpu\(\)\.numpy\(\)\.astype\(np\.float32\)/\1.float().cpu().numpy()/' \
        -e 's/\.clamp\(0\.0, 1\.0\)\.cpu\(\)\.numpy\(\)/.clamp(0.0, 1.0).float().cpu().numpy()/' \
        "$f"; \
    test "$(grep -c '\.cpu()\.numpy()\.astype(np\.float32)' "$f")" = 0; \
    test "$(grep -c '\.float()\.cpu()\.numpy()' "$f")" = 6
...
RUN { echo "comfyui=$(git -C /opt/ComfyUI rev-parse HEAD)"; \
      ...
      echo "patch_bf16_numpy=$(grep -q 'voxel_colors.detach().float().cpu().numpy()' /opt/ComfyUI/comfy_extras/nodes_mesh_postprocess.py && grep -c '\.float()\.cpu()\.numpy()' /opt/ComfyUI/comfy_extras/nodes_save_3d.py | grep -qx 6 && echo yes || echo no)"; \
